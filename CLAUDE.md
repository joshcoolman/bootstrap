...
The value of this repo is not code templates — models can write a Vite app.
...
1. **Scaffold a mule.** Fresh folder, `/bootstrap:vite-app` (dogfoods the
   shell skill every time). For a skill that layers on something other than
   the shell, start from whatever it layers on.
...
5. **Integration-test the skill as a consumer**: fresh folder,
   ...
   (`/bootstrap:vite-app` → `/bootstrap:add-<x>`). Use `/reload-plugins` to
   iterate on skill edits mid-session.
...
- **Assume the stack, discover the shell.** Vite + React + TanStack Router +
...
- Vendor-specific code stays behind seams the skill itself establishes (e.g.
  auth's `AuthClient` contract: a second vendor is one new adapter + a wizard
...
- `parts/` + `recipes/` — the same knowledge as human-readable instruction
  docs. When a skill and a part cover the same ground, the skill is the
  source of truth for exact code; keep the part at instruction level.
